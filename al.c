...
int stop_game_check_result(t_game *game)
{
    int res;
    res  = 1;
    if (game->stop_game.curr_fit_position > game->stop_game.offset_to_triger_zone + game->stop_game.triger_zone_len ||
		game->stop_game.curr_fit_position < game->stop_game.offset_to_triger_zone)
	{
        lose_game();
        res = -1;
	}
...
int stop_game_render_new_frame(t_game *game)
{
    ...
    srand(time(NULL));
    game->stop_game.offset_to_triger_zone = 300 + rand() % (500 - game->stop_game.triger_zone_len);
    i = game->stop_game.offset_to_triger_zone;
    ...
    game->stop_game.curr_fit_position += (800 - game->meters_left) / 200 + 1;
...
void    stop_game(int step_counter, int curr_position, int round_time, t_game *game)
{
    ...
    (void)round_time;
    (void)curr_position;
    stop_game_render_new_frame(game);
}
